...
- The root `<html lang>` attribute is fixed at `"en"` in the initial static HTML for every route (the single root `app/layout.tsx` renders before the `[locale]` segment beneath it resolves, so it cannot read the locale param). A client component (`SetHtmlLang`) corrects it to the real locale immediately on hydration, which is enough for real browsers/screen readers, but a crawler or no-JS client reading raw HTML on `/es/...` will see `lang="en"` in the initial markup. Fixing this properly would require middleware (unavailable in a static export) or making `[locale]` the true route root, which conflicts with keeping `/` as an unprefixed gateway page.
